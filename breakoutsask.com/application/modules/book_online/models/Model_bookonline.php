<?php
class Model_bookonline extends CI_Model{
	...
	public function get_homepg_middle_box(){
		...
		if($query = $this->db->query("SELECT * FROM `".$this->db->dbprefix('cms_pages')."` WHERE `is_active`='1' and `site_id`='".$this->site_id."' and `parent_id`='".$id."'  order by display_order > 0 desc,display_order asc  "))
		{
			#echo $this->db->last_query();
			$res = $query->result();
			if(!empty($res)){
				...
				if($query = $this->db->query("SELECT * FROM `".$this->db->dbprefix('cms_pages')."` WHERE `is_active`='1' and `site_id`='".$this->site_id."' and `parent_id`='".$parent_id."' order by display_order > 0 desc,display_order asc  "))
				{
					return $query->result();
				}else{
					return false;
