...
class Homepage_block_setting extends MY_Controller {
	private $limit = 20;
	var $offset = 0;
	function __construct()
	{
		...
		$this->load->model('modelhomepage_block_setting');
	...
	public function pagination($searchstring="",$total_row=0){
		...
		if(!empty($searchstring)){
			...
			$config['base_url'] = site_url("homepage_block_setting/dosearch/".rawurlencode($searchstring)."/");
		}
		else{
			$config['uri_segment'] = 4;
			$config['base_url'] = site_url("homepage_block_setting/index/");
		}
	...
	public function dolist($searchstring=""){
		$data = array();
		$total_row = $this->modelhomepage_block_setting->getCountAll("homepage_block_setting",$searchstring);
		$this->pagination($searchstring,$total_row);
		$data['q'] = $searchstring;
		
		$data_row = $this->modelhomepage_block_setting->getAllDetails("homepage_block_setting",$this->limit,$this->offset,$searchstring);
		if(empty($data_row)){
		...
		$this->load->view('homepage_blocks_setting_list',$data);		
	}
	...
	public function doadd(){
		...
		$data['totalsection1'] = $this->modelhomepage_block_setting->getSectiontotal("homepage_block_setting",'1');
		$data['totalsection2'] = $this->modelhomepage_block_setting->getSectiontotal("homepage_block_setting",'2');
		$data['totalsection3'] = $this->modelhomepage_block_setting->getSectiontotal("homepage_block_setting",'3');
		$data['totalsection4'] = $this->modelhomepage_block_setting->getSectiontotal("homepage_block_setting",'4');
		//$data['category']= $this->productcategoryselectbox("0",'','2');
		//$data['selected_category']= '';
		/*$data_row = $this->modelcategory->getAllCategory("products");
		$data['records']= $data_row;*/
		
		$this->load->view('edit_homepage_block_setting',$data);		
	}
	public function addedit()
	{
		...
		{
			if(!empty($id) && $this->modelhomepage_block_setting->getSingleRecord($id)) 
			{	

				$getFile=$this->modelhomepage_block_setting->editDetail($id);
				$orgimgpath=$getFile->block_background_image;
				if(isset($_FILES['htmlfile']['tmp_name']) && is_uploaded_file($_FILES['htmlfile']['tmp_name'])) // if image file upload at the updating
				{
					...
					if(!empty($orgimgpath) && is_file(file_upload_absolute_path().'homepage_background_image/bg_'.$orgimgpath)){
						unlink(file_upload_absolute_path().'homepage_background_image/bg_'.$orgimgpath);
					}
									
					$uplod_img=$this->uploadImage('htmlfile','homepage_background_image/');
					
					 
					$this->resizingImage($uplod_img,$upload_rootdir='homepage_background_image/',$img_width='2000',$img_height='1490',$img_prefix = 'bg_');
				...
				if($this->modelhomepage_block_setting->updateDetais($id,$uplod_img)) // IF UPDATE PROCEDURE EXECUTE SUCCESSFULLY
				{
				...
				if(is_uploaded_file($_FILES['htmlfile']['tmp_name'])) // if image file upload at the updating
			{
				...
				if(empty($uplod_img) || $uplod_img==false)
				{
					...
					$this->session->set_userdata($session_data);	
					redirect("kaizen/homepage_block_setting/doedit/".$id,'refresh');
				}
			}
				$id = $this->modelhomepage_block_setting->addDetails($uplod_img);
	...
	public function doedit()
	{
		...
		$q = $this->modelhomepage_block_setting->editDetail($banner_id);		
		...
		//$data_row = $this->modelhomepage_block_setting->getAllDetails("homepage_block_setting");
		//$data['records']= $data_row;
		...
		public function do_changestatus() // CHANGE STATUS
	{
		...
		if((int)$status_id > 0 && $this->modelhomepage_block_setting->getSingleRecord($status_id))
		{ 
			if($this->modelhomepage_block_setting->changeStatus('homepage_block_setting',$status_id))
			{				
		...
		redirect("kaizen/homepage_block_setting",'refresh');
	...
	public function dodelete($del_id){		
		...
		$tablename="homepage_block_setting";
		$idname='id';
			
		$delrec=$this->modelhomepage_block_setting->delSingleRecord($del_id,$tablename,$idname);
		if($delrec===true){
			echo "Success";
		...
		//redirect("kaizen/homepage_block_setting/dolist/",'refresh');		
	...
	$field = $this->input->get('field');
		...
		if((int)$id > 0 && $this->modelhomepage_block_setting->getSingleRecord($id))
		{ 
			if($this->modelhomepage_block_setting->deleteImg($id,$field))
			{				
		...
		else
		{
			$session_data = array("ERROR_MSG"  => "Image Not deleted Successfully.");
			$this->session->set_userdata($session_data);
			redirect("kaizen/homepage_block_setting/doadd/0","refresh");
		}
