...
class Homepage_slider extends MY_Controller {
	private $limit = 20;
	var $offset = 0;
	function __construct()
	{
		...
		$this->load->model('modelhomepage_slider');
	...
	public function pagination($searchstring="",$total_row=0){
		...
		if(!empty($searchstring)){
			...
			$config['base_url'] = site_url("homepage_slider/dosearch/".rawurlencode($searchstring)."/");
		}
		else{
			$config['uri_segment'] = 4;
			$config['base_url'] = site_url("homepage_slider/index/");
		}
	...
	public function dolist($searchstring=""){
		$data = array();
		$total_row = $this->modelhomepage_slider->getCountAll("homepage_slider",$searchstring);
		$this->pagination($searchstring,$total_row);
		$data['q'] = $searchstring;
		
		$data_row = $this->modelhomepage_slider->getAllDetails("homepage_slider",$this->limit,$this->offset,$searchstring);
		if(empty($data_row)){
		...
		$this->load->view('homepage_slider_list',$data);		
	}
	...
	public function doadd(){
		...
		$this->load->view('edit_homepage_slider',$data);		
	}
	public function addedit()
	{
		...
		{
			if(!empty($id) && $this->modelhomepage_slider->getSingleRecord($id)) 
			{	

				$getFile=$this->modelhomepage_slider->editDetail($id);
				$orgimgpath=$getFile->slider_image;
...
//						unlink(file_upload_absolute_path().'slider_image/bg_'.$orgimgpath);
//					}
									
					$uplod_img=$this->uploadImage('htmlfile','slider_image/');
					
					 
					//$this->resizingImage($uplod_img,$upload_rootdir='slider_image/',$img_width='2000',$img_height='1490',$img_prefix = 'bg_');
				...
				if($this->modelhomepage_slider->updateDetais($id,$uplod_img)) // IF UPDATE PROCEDURE EXECUTE SUCCESSFULLY
				{
			...
			//	$this->resizingImage($uplod_img,$upload_rootdir='slider_image/',$img_width='2000',$img_height='1490',$img_prefix = 'bg_');
				...
				if(empty($uplod_img) || $uplod_img==false)
				{
					...
					$this->session->set_userdata($session_data);	
					redirect("kaizen/homepage_slider/doedit/".$id,'refresh');
				}
			}
				$id = $this->modelhomepage_slider->addDetails($uplod_img);
	...
	public function doedit()
	{
		...
		$q = $this->modelhomepage_slider->editDetail($banner_id);		
		...
		$data_row = $this->modelhomepage_slider->getAllDetails("homepage_slider");
		$data['records']= $data_row;
		...
		public function do_changestatus() // CHANGE STATUS
	{
		...
		if((int)$status_id > 0 && $this->modelhomepage_slider->getSingleRecord1('homepage_slider',$status_id))
		{ 
			if($this->modelhomepage_slider->changeStatus('homepage_slider',$status_id))
			{				
		...
		redirect("kaizen/homepage_slider",'refresh');
	...
	public function dodelete($id){		
		...
		$tablename="homepage_slider";
		$idname='id';
			
		$delrec=$this->modelhomepage_slider->delSingleRecord($id,$tablename,$idname);
		if($delrec===true){
			echo "Success";
	...
	//	redirect("kaizen/homepage_slider/dolist/",'refresh');		
	...
	$field = $this->input->get('field');
		...
		if((int)$id > 0 && $this->modelhomepage_slider->getSingleRecord($id))
		{ 
			if($this->modelhomepage_slider->deleteImg($id,$field))
			{				
		...
		else
		{
			$session_data = array("ERROR_MSG"  => "Image Not deleted Successfully.");
			$this->session->set_userdata($session_data);
			redirect("kaizen/homepage_slider/doadd/0","refresh");
		}
